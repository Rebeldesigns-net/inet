...
void TimingMeasurementMaker::processPacket(Packet *packet)
{
    if (packetFilter.matches(packet)) {
            ...
            data->mapAllTags<ElapsedTimeTag>(offset, length, [&] (b offset, b length, const ElapsedTimeTag *timeTag) {
                for (int i = 0; i < (int)timeTag->getTotalTimesArraySize(); i++) {
                    ...
                    if (flowNameMatcher.matches(&matchableFlowName)) {
                        cNamedObject signalDetails(flowName);
                        makeMeasurement(packet, offset, length, flowName, elapsedTimeSignal, simTime() - timeTag->getTotalTimes(i));
                    }
                }
            });
        }
        ...
        if (finish) {
            ...
            // KLUDGE:
            cStringTokenizer tokenizer(flowName);
            while (tokenizer.hasMoreTokens())
                endPacketFlow(this, packet, tokenizer.nextToken());
        }
    }
}
