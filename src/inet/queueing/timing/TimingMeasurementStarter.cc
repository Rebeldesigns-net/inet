...
void TimingMeasurementStarter::processPacket(Packet *packet)
{
    if (packetFilter.matches(packet)) {
        ...
        if (measureElapsedTime)
            startMeasurement<ElapsedTimeTag>(packet, data, offset, length, flowName, simTime());
        if (measureDelayingTime)
            startMeasurement<DelayingTimeTag>(packet, data, offset, length, flowName, 0);
        if (measureQueueingTime)
            startMeasurement<QueueingTimeTag>(packet, data, offset, length, flowName, 0);
        if (measureProcessingTime)
            startMeasurement<ProcessingTimeTag>(packet, data, offset, length, flowName, 0);
        if (measureTransmissionTime)
            startMeasurement<TransmissionTimeTag>(packet, data, offset, length, flowName, 0);
        if (measurePropagationTime)
            startMeasurement<PropagationTimeTag>(packet, data, offset, length, flowName, 0);
        {
            ...
            data->addTagsWhereAbsent<PacketEventTag>(offset, length);
        }
