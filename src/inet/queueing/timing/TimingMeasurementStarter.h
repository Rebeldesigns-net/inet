...
class INET_API TimingMeasurementStarter : public PacketFlowBase
{
  ...
  protected:
    ...
    template <typename T>
    void startMeasurement(Packet *packet, const Ptr<Chunk>& data, b offset, b length, const char *flowName, simtime_t value) {
        if (length == b(-1))
        ...
        data->addTagsWhereAbsent<T>(offset, length);
        data->mapAllTags<T>(offset, length, [&] (b o, b l, T *timeTag) {
